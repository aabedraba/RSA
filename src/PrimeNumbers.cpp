 ...
 * @param range const long, exponente a elevar el rango
 * @post dos números primos en el par de variables _primos
 */
void PrimeNumbers::generatePrimes( const long range ) {
    gmp_randstate_t state1;
    gmp_randinit_mt( state1 ); // Merssene-Twister initializer; good pseudo-randomness properties
    gmp_randstate_t state2;
    gmp_randinit_default( state2 ); // using different initializer
    mpz_rrandomb( _primes.first, state1, range );
    mpz_rrandomb( _primes.second, state2, range + 1 );
...
bool PrimeNumbers::isPrime( mpz_t *n ) {
    ...
    for ( int i = 0; i < millersTestIterations; ++i )
        if ( !millerRabinsTest( d1, n )) isPrime = false;
